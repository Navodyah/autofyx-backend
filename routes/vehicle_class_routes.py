...
def create_vehicle_class_route(vehicle_class: VehicleClassCreate, db: Session = Depends(get_db)):
    """Create a new vehicle class"""
    return create_vehicle_class(db, vehicle_class.class_name)


@router.get("/{class_id}", response_model=VehicleClassResponse)
def get_vehicle_class_route(class_id: int, db: Session = Depends(get_db)):
    """Get a vehicle class by ID"""
    vehicle_class = get_vehicle_class_by_id(db, class_id)
...
def get_all_vehicle_classes_route(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """Get all vehicle classes"""
    return get_all_vehicle_classes(db, skip, limit)


@router.put("/{class_id}", response_model=VehicleClassResponse)
def update_vehicle_class_route(class_id: int, vehicle_class: VehicleClassUpdate, db: Session = Depends(get_db)):
    """Update a vehicle class"""
    updated_vehicle_class = update_vehicle_class(db, class_id, vehicle_class.class_name)
...
def delete_vehicle_class_route(class_id: int, db: Session = Depends(get_db)):
    """Delete a vehicle class"""
    success = delete_vehicle_class(db, class_id)
    if not success:
        raise HTTPException(status_code=404, detail="Vehicle class not found")
    return None
